...
package starling.display
{
	...
	public class DisplayObjectContainer extends DisplayObject
	{
		...
		private var mTouchableChildren:Boolean = true;
		private var mBroadCastEnabled:Boolean = true;
		...
		{
			addChildAt(child, mChildren.length);
			return child;
		}
		...
		public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle = null):Rectangle
		{
			...
			{
				child = mChildren[0];
				if (child.includeInParentBounds == false)
				{
					getTransformationMatrix(targetSpace, sHelperMatrix);
					MatrixUtil.transformCoords(sHelperMatrix, 0.0, 0.0, sHelperPoint);
					resultRect.setTo(sHelperPoint.x, sHelperPoint.y, 0, 0);
		...
		public function set children(v:Vector.<DisplayObject>):void
		{
			if (v == null)
			{
		...
		public function get broadCastEnabled():Boolean
		{
			return mBroadCastEnabled;
		}
		
		public function set broadCastEnabled(value:Boolean):void
		{
			mBroadCastEnabled = value;
		}
		
		public function get touchableChildren():Boolean
		{
			return mTouchableChildren;
		}
		
		public function set touchableChildren(value:Boolean):void
		{
			mTouchableChildren = value;
		}
		...
		public function AddChild(child:DisplayObject, dispatchAdd:Boolean = false, dispatchAddToStage:Boolean = false):DisplayObject
		{
			AddChildAt(child, mChildren.length, dispatchAdd, dispatchAddToStage);
			return child;
		}
		
		public function RemoveChild(child:DisplayObject, dispatchRemove:Boolean = false, dispatchRemoveFromStage:Boolean = false, dispose:Boolean = false):DisplayObject
		{
			var childIndex:int = getChildIndex(child);
			if (childIndex != -1)
			{
				RemoveChildAt(childIndex, dispatchRemove, dispatchRemoveFromStage, dispose);
			}
			return child;
		}
		
		public function RemoveChildren(beginIndex:int = 0, endIndex:int = -1, dispatchRemove:Boolean = false, dispatchRemoveFromStage:Boolean = false, dispose:Boolean = false):void
		{
			if (endIndex < 0 || endIndex >= mChildren.length)
			{
				endIndex = mChildren.length - 1;
			...
			for (var i:int = beginIndex; i <= endIndex; ++i)
			{
				RemoveChildAt(beginIndex, dispatchRemove, dispatchRemoveFromStage, dispose);
			}
		}
		
		public function AddChildAt(child:DisplayObject, index:int, dispatchAdd:Boolean = false, dispatchAddToStage:Boolean = false):DisplayObject
		{
			var numChildren:int = mChildren.length;
		...
		public function Clear(dispose:Boolean = false, dispatchRemove:Boolean = false, dispatchRemoveFromStage:Boolean = false):void
		{
			var len:int = mChildren.length;
			var child:DisplayObject;
			var isStage:Boolean = dispatchRemoveFromStage && stage != null;
		...
		public function RemoveChildAt(index:int, dispatchRemove:Boolean = false, dispatchRemoveFromStage:Boolean = false, dispose:Boolean = false):DisplayObject
		{
			if (index >= 0 && index < mChildren.length)
			{
				var child:DisplayObject = mChildren[index];
				...
				if (dispatchRemoveFromStage && stage)
				{
					var container:DisplayObjectContainer = child as DisplayObjectContainer;
					if (container)
					{
