// =================================================================================================
//
//	Copyright 2012, DoItFlash. All rights reserved.
//
...
package starling.utils.scroller
{
...
// ----------------------------------------------------------------------------------------------------------------------- Methods
		...
		public function startScroll($point:Point):void
		{
			...
			if (_isScrollBegin) // _isScrollBegin is true when user scrolls for the first time and each time he calls fling()
			{
				...
				if (hasEventListener(ScrollEvent.MOUSE_DOWN)) {
			...
			if (hasEventListener(ScrollEvent.MOUSE_MOVE)) {
		...
		public function fling():void
		{
			...
			if (hasEventListener(ScrollEvent.MOUSE_UP)) {
			...
			ThrowPropsPlugin.to(_content, {throwProps:{
								y:{velocity:yVelocity, max:0, min:0 - _yOverlap, resistance:300},
								x:{velocity:xVelocity, max:0, min:0 - _xOverlap, resistance:300}
		...
		private function onTweenUpdate():void
		{
			computeYPerc();
			computeXPerc();
			if (hasEventListener(ScrollEvent.TOUCH_TWEEN_UPDATE)) {
		...
		private function onTweenComplete():void
		{
			...
			if (hasEventListener(ScrollEvent.TOUCH_TWEEN_COMPLETE)) {
		...
		public function computeYPerc($manualPerc:Boolean = false):void
		{
			...
			if ($manualPerc)
			{
				...
				TweenMax.to(_content, _duration, { bezier:[ { y:_content.y }, { y: - yLoc } ], ease:_easeTypeFunc } );
				$manualPerc = false;
			}
			else
			{
