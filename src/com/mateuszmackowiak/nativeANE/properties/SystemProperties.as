package com.mateuszmackowiak.nativeANE.properties
{
	...
	import com.mateuszmackowiak.nativeANE.alert.NativeAlert;
	...
	public class SystemProperties extends EventDispatcher
	{
		...
		{
			try{
				_context = ExtensionContext.createExtensionContext(NativeAlert.EXTENSION_ID, 'SystemProperites');
				_context.addEventListener(StatusEvent.STATUS, onStatus);
			}catch(e:Error){
				throw("SystemProperties initialization error : "+e.message,e.errorID);
			}
		}
		...
		[Deprecated(replacement="SystemProperties.getInstance().getProperites()")] 
		/**
		 * Returns Dictionary object with additional system properties that are missing in Adobe AIR
		 */
		...
		{
			try{
				var context:ExtensionContext = ExtensionContext.createExtensionContext(NativeAlert.EXTENSION_ID, 'SystemProperites');
