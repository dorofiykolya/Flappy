...
package com.junkbyte.console {
	...
	public class Cc{
		 ...
		 */
		public static function start(container:DisplayObjectContainer, password:String = "", onAddToStage:Function = null):void{
			if(_console){
				if(container && !_console.parent) container.addChild(_console);
			}else{
				_console = new Console(password, config);
				if (onAddToStage as Function)
				{
					_console.addEventListener(Event.ADDED_TO_STAGE, onAddToStage);
				}
		...
		public static function log(...strings):void {
			if (_console) 
			{
				if (_logs.length > 0)
				{
					...
					{
						_console.log.apply(null, _logs[i]);
					}
					_logs.length = 0;
				}
